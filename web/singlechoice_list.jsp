...
<table width="100%" border="2" align="center" cellpadding="6" cellspacing="3" bordercolor="SkyBlue" style="border-collapse:collapse">
    ...
    <%
        ...
        String fysql="select count(id) as ss from singlechoice";
...
//        }
        ...
        sql="";
        sql="select top "+page_record+" * from singlechoice where id not in (select top "+((curpage-1)*page_record)+" id from singlechoice order by id desc) ";
        ...
        if(request.getParameter("question")=="" ||request.getParameter("question")==null ){}
        else{sql=sql+" and question like '%"+new String(request.getParameter("question").getBytes("8859_1"))+"%'";}
        if(request.getParameter("difficult")=="所有" ||request.getParameter("difficult")==null ){}
        else{sql=sql+" and difficult like '%"+new String(request.getParameter("difficult").getBytes("8859_1"))+"%'";}
...
//            updateTime=RS_result.getString("updateTime");
    ...
    <tr>
        ...
        <td width="60" align="center">
            <a href="singlechoice_update.jsp?id=<%=id%>">修改</a>
            <a href="del.jsp?id=<%=id %>&tablename=singlechoice" onClick="return confirm('真的要删除？')">删除</a>
        </td>
    </tr>
<%--    <%--%>
<%--        }--%>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="singlechoice_list.jsp?page=1">首页</a>　
    <a href="singlechoice_list.jsp?page=<%= curpage-1%>">上一页</a>　
    <a href="singlechoice_list.jsp?page=<%= curpage+1%>">下一页</a>　
    <a href="singlechoice_list.jsp?page=<%=zys %>">尾页</a>　
